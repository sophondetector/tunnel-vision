...
async function getLatestPDFUrl(): Promise<string> {
  ...
  return res[LATEST_PDF_URL_KEY] as string
}
...
VOLUME_SLIDER.addEventListener("input", async (event) => {
  ...
  const dir = getDirector()
  await dir.setSoundVol(value)
  VOLUME_DISPLAY.textContent = `${value}%`
  await dir.playSound()
  // NOTE: this is here to make sure that user pressing "alt+arrow" doesn't also count as volume input; it effectively disables keyboard control for the volume slider
  VOLUME_SLIDER.blur()
})
...
OPACITY_SLIDER.addEventListener('input', async (event) => {
  ...
  // NOTE: this is here to make sure that user pressing "alt+arrow" doesn't also count as opacity input; it effectively disables keyboard control for the opacity slider
  OPACITY_SLIDER.blur()
})
...
getPDFDocumentProxy()
  ...
  .then(async () => {
    const dir = getDirector()
    return dir.soundIsOn()
  })
  .then(displaySoundIsOn)
  .then(async () => {
    const dir = getDirector()
    return dir.getSoundVol()
  })
  .then((vol) => {
