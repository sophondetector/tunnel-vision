...
export class HandlerManager {
  static setHandler(handler: TvHandler): void {
    HANDLER = handler
  }
  ...
  static async getScrollableElement(resetScrollable: boolean = false): Promise<Element | null> {
    if (SCROLLABLE_ELEMENT && resetScrollable === false) {
      return SCROLLABLE_ELEMENT
    }

    SCROLLABLE_ELEMENT = await HandlerManager.getHandler().getScrollableElement()
    return SCROLLABLE_ELEMENT
  }
